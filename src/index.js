...
function addChore() {
  ...
  if (
    form.choreName.value == "" ||
    form.chorePrio.value == "" ||
    form.choreTime.value == "" ||
    form.chorePrio.value <= 0 ||
    form.choreTime.value <= 0
  ) {
    showFormatError();
  } else {
    let id = ChoreLogic.addChore(
      form.choreName.value,
      form.choreTime.value,
      form.chorePrio.value
    );
...
function showFormatError() {
  alert(
    "Please fill all the Boxes with the correct values (only positive numbers)"
  );
}
...
function renderToDo(id) {
  ...
  choreTime.innerText = `Estimated time: ${Math.floor(
    ChoreLogic.choresArray[getIndex(id)].esTime
  )} minutes.`;
  choreTime.className = "choreTime";
  let chorePrio = document.createElement("p");
  chorePrio.innerText = `Priority: ${
    ChoreLogic.choresArray[getIndex(id)].priority
  }.`;
  chorePrio.className = "chorePrio";
  let choreCreation = document.createElement("p");
  let creation = date.format(
    ChoreLogic.choresArray[getIndex(id)].creation,
    pattern
  );
  ...
  choreStatus.innerText = `Status: ${
    ChoreLogic.choresArray[getIndex(id)].status
  }.`;
  ...
  changeChore.addEventListener("click", () => {
    if (btnControl == true) {
      ...
      let nextInt = document.querySelector(
        `#choresList li:nth-child(${getIndex(id) + 1})`
      );
